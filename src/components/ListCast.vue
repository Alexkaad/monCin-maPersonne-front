...
const props = defineProps<{ perform: CastMember[] }>();
const isExpanded = ref(false);

const safePerform = computed(() => {
  return [...props.perform]
      .sort((a, b) => {
...
<template>
  <table v-if="safePerform && safePerform.length > 0"
      ...
      <td>
        <div class="d-flex flex-column align-items-start">
          ...
          <p style="width:15rem">
            ...
            <span v-if="filmItem.character" class="fw-light">{{ filmItem.character }}</span>
            <span v-else>----</span>
          </p>
